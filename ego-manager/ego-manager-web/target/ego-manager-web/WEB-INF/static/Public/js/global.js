 ...
 *  获取多级联动的商品分类
 * @param id  选中的下拉框的值
 * @param next      下级菜单的id名称
 */
function get_category(id,nextId,select_id){
    ...
    if(0==id){
        ...
        var str=nextId.split("_");
        if(parseInt(str[2])-2>0){
            var preId=str[0]+"_"+str[1]+"_"+(parseInt(str[2])-2);
            $("#"+select_id).val($("#"+preId).val());
        }
        if(0==parseInt(str[2])-2){
            var next_next_str=str[0]+"_"+str[1]+"_"+(parseInt(str[2])+1);
            $("#"+next_next_str).html(html);
        }
        return;
    }
    ...
    var url=getProjectName()+"/product/category/byParentId";
    $.ajax({
        url: url,
        type: "post",
        data: {parentId: id},
...
//设置隐藏域的值
function setHidden(id,preId,select_id){
        $("#"+select_id).val(id);
        if(0==id){
            $("#"+select_id).val($("#"+preId).val());
        }
