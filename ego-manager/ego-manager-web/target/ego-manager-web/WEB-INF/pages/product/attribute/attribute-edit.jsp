                ...
                <div class="panel-body">
                    ...
                    <div class="tab-content">                 	  
                        <div class="tab-pane active" id="tab_tongyong">
                           <input type="text" name="attrId" value="${goodsAttribute.attrId}"/>
                            <table class="table table-bordered">
                                <tbody>
                                <tr>
                                    <td>属性名称：</td>
                                    <td>
                                        <input value="${goodsAttribute.attrName}" name="attrName" type="text">
                                        <span id="err_attr_name" style="color:#F00; display:none;"></span>                                        
                                    </td>
                                </tr>  
                                <tr>
                                    <td>所属商品类型：</td>
                                    <input type="text" value="${goodsAttribute.typeId}" name="typeId" id="typeId"/>
                                    <input type="text" value="${goodsAttribute.typeName}" name="typeName" id="typeName"/>
                                    <td>
                                        <select name="typeId" id="type_id" onchange="changeTypeIdAndName()" >
                                             <option value="">请选择</option>
                                            <c:forEach items="${goodsTypeList}" var="goodsType">
                                                 <option  <c:if test="${goodsAttribute.typeId == goodsType.id}">selected</c:if>
                                ...
                                <tr>
                                    <td>能否进行检索：</td>
                                    <td>
                                        <input value="0" name="attrIndex" <c:if test="${0==goodsAttribute.attrIndex}">checked="checked"</c:if>  type="radio">不需要检索
                                        <input value="1" name="attrIndex"  <c:if test="${1==goodsAttribute.attrIndex}">checked="checked"</c:if> type="radio">关键字检索
                                ...
                                <tr>
                                    <td>该属性值的录入方式：</td>
                                    <td>
                                        <input value="0" name="attrInputType"  <c:if test="${0==goodsAttribute.attrInputType}">checked="checked"</c:if> type="radio">手工录入
                                        <input value="1" name="attrInputType" <c:if test="${1==goodsAttribute.attrInputType}">checked="checked"</c:if> type="radio">从下面的列表中选择（一行代表一个可选值）
                                        <input value="2" name="attrInputType" <c:if test="${2==goodsAttribute.attrInputType}">checked="checked"</c:if> type="radio">多行文本框
                                    </td>
                                </tr>  
                                <tr>
                                    <td>可选值列表：</td> 
                                    <td>
                                    <textarea rows="5" cols="30" name="attrValues">${goodsAttribute.attrValues}</textarea>
